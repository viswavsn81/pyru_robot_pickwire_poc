...
def main():
    ...
    while True:
        ...
        # --- F. SEND COMMAND (AS DICTIONARY!) ---
        # The robot expects {'motor_name.pos': value, ...}
        ...
        for name in motor_names:
            ...
            if name == "gripper":
                # Manual Gripper Control: -1..1 -> 0..90 degrees
                angle = 0 + (gripper_state * 45) + 45 
            elif name in arm_joints:
                # Math Controlled
                idx = arm_joints.index(name)
                angle = target_joints_deg[idx]
            
            # Pack into dictionary using the Tensor wrapper if needed
            # But usually send_action handles raw floats if in a dict.
            # Let's wrap in tensor just to be safe as per LeRobot standard.
