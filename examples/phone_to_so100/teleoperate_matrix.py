...
def make_4x4_matrix(pos, quat):
    """
    Manually builds a 4x4 Homogeneous Transformation Matrix
    from a Position [x,y,z] and Quaternion [x,y,z,w].
    This satisfies the strict C++ requirements of the solver.
    """
    x, y, z = pos
    qx, qy, qz, qw = quat
    
    # Formula to convert Quaternion to Rotation Matrix
    # (Standard 3D Math, no external libraries needed)
...
def main():
    ...
    while True:
        ...
        # 1. Build the strict 4x4 matrix the solver demands
        target_matrix = make_4x4_matrix(target_pos, target_quat)
        
        # 2. Pass it (Solver sees: joints, 4x4_matrix)
